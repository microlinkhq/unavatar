...
test('gravatar hashes email with sha256', t => {
  ...
  t.is(
    gravatar(email),
    `https://gravatar.com/avatar/${hash}?size=${AVATAR_SIZE}&d=404`
  )
})
...
test('gravatar uses md5 hash input directly (legacy)', t => {
  ...
  const md5hash = crypto
    .createHash('md5')
    .update('hello@example.com')
    .digest('hex')
  t.is(
    gravatar(md5hash),
    `https://gravatar.com/avatar/${md5hash}?size=${AVATAR_SIZE}&d=404`
  )
})
